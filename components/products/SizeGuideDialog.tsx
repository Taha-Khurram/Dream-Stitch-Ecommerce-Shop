 ...
 * touching this file.
 *
 * The overlay is portalled to `<body>` rather than rendered where it sits.
 * Every storefront page is wrapped in `.page-in`, and that animation finishes
 * `both`, so the wrapper keeps an identity transform for the life of the page
 * — enough to make it, and not the viewport, the containing block for anything
 * `position: fixed` inside it. Left in place the backdrop covered the article
 * instead of the window and the panel was centred on the whole scrollable
 * page, which is how the guide came to open somewhere down the middle of it.
 */
export function SizeGuideDialog({ guide }: { guide: SizeGuide }) {
  ...
  return (
    <>
      ...
      {open &&
        createPortal(
          /* Anchored to the top rather than centred: the link that opens it
             sits high in the buy box, and a long chart centred on a short
             viewport has nowhere to grow. The backdrop scrolls, so it always
             does. */
          <div
